...
class InputPublisher(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(5, self.publish_input_data)
    ...
    def publish_input_data(self):
        ...
        tmp_sec = current_time % 1000.0

        # 임의의 입력 데이터 생성
        input_data = [tmp_sec] + np.random.rand(1, 10).flatten().tolist()

        # 메시지에 타임스탬프와 데이터 포함
        self.msg.data = input_data
        
        # 순차적으로 각 모델에 데이터 전송        
        self.input_publishers[0].publish(self.msg)
        self.input_publishers[1].publish(self.msg)
        self.input_publishers[2].publish(self.msg)
        self.input_publishers[3].publish(self.msg)
